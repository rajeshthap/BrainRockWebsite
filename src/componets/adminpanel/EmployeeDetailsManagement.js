...
const EmployeeDetailsManagement = () => {
  ...
  const generateEmployeePdfContent = (employee) => {
    ...
    } else if (isUSInfotechEmployee) {
      pdfTitle = "U.S. Infotech Employee Details";
    } else {
      pdfTitle = `${firmName} Employee Details`;
    }
      ...
      <head>
          ...
          <style>
              ...
              .profile-pic-header {
                  width: 60px;
                  height: 60px;
                  border-radius: ${isZeeEmployee ? "8px" : "50%"}; /* Diksha photo is not in header */
                  object-fit: cover;
                  border: 3px solid #e6edf7;
              }
              .profile-pic-placeholder-header {
                  width: 60px; height: 60px; border-radius: ${isZeeEmployee ? "8px" : "50%"}; background-color: #e0e0e0; /* Diksha photo is not in header */
  ...
  const generateFirmWisePdfContent = (firmName, employees) => {
    const escapeHtml = (value) =>
      String(value ?? "")
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
    ...
    const firmDisplayName = firmName === "all" ? "All Firms" : firmName;
    const isZeeEmployee = firmName === "ZEE - Zero Error Enterprises";
    const isDikshaEmployee = firmName === "Diksha Enterprises";
    const isUSInfotechEmployee = firmName === "U.S. Infotech";
    const isBrainrockEmployee = firmName === "Brainrock Consulting Services";
    ...
    const employeeRows = employees
      .map((emp, index) => `
        <tr>
          ...
          <td>${escapeHtml(emp.emp_name || "N/A")}</td>
    ...
    return `
      <html>
      <head>
          <title>${firmDisplayName} - Employee Report</title>
          <style>
              body {
                  font-family: Arial, sans-serif;
                  margin: 0;
              ...
              .pdf-header-center .section-title {
                  margin: 0;
                  font-size: 16px;
                  color: ${accentColor};
                  font-weight: 700;
      ...
      <body>
          <div class="pdf-shell">
               <div class="pdf-header">
                   ...
                   <div class="pdf-header-center">
                       <h2 class="section-title">Employee List - ${escapeHtml(firmDisplayName)}</h2>
                   </div>
                   <div style="display: flex; align-items: center; gap: 12px;">
                       <div class="pdf-subtitle">Generated on ${new Date().toLocaleDateString()} | Total Employees: ${employees.length}</div>
                       <button class="print-btn" onclick="window.print()">Print Report</button>
                   </div>
               </div>
               <div class="pdf-body">
                   <table>
                      <thead>
                           <tr>
                               ...
                               <th style="width: 25%;">Work Experience</th>
  ...
  const handleFirmWisePdf = () => {
    const employeesToUse =
      selectedFirmFilter === "all" ? allEmployees : filteredEmployees;

    if (employeesToUse.length === 0) {
      setMessage("No employees found for the selected firm.");
      setVariant("danger");
      setShowAlert(true);
      return;
    }

    const htmlContent = generateFirmWisePdfContent(
      selectedFirmFilter,
      employeesToUse,
    );
    const printWindow = window.open("", "_blank");
  ...
  const handleFirmWiseExcel = () => {
    ...
    const firmName =
      selectedFirmFilter === "all"
        ? "All_Firms"
        : selectedFirmFilter.replace(/[^a-zA-Z0-9]/g, "_");
    XLSX.writeFile(
      wb,
      `${firmName}_Employees_${new Date().toISOString().split("T")[0]}.xlsx`,
    );
  };

  const handleFirmWisePrint = () => {
    const employeesToUse =
      selectedFirmFilter === "all" ? allEmployees : filteredEmployees;
